...
export interface FeishuCardBuilderStrings {
  ...
  mergeSummaryOthers(count: number, list: string): string;
  ...
  mergeSummarySubtitle(branchName: string, baseBranch: string, accepted: number, total: number): string;
...
const zhCN: FeishuCardBuilderStrings = {
  ...
  mergeReviewRejectAction: "❌ 拒绝 + 反馈 Agent",
  mergeReviewAcceptAll: (remaining) => `**全部接受** (剩余 ${remaining} 个文件)`,
  mergeReviewProgress: (accepted, rejected, remaining, pct) => `进度: ✅${accepted}  ❌${rejected}  ⏳${remaining} (${pct}%)`,
  mergeSummaryAccepted: (count, list) => `**接受 (${count})**\n${list}`,
  mergeSummaryOthers: (count, list) => `**其他 (${count})**\n${list}`,
  mergeSummaryPartialWarning: "⚠️ 部分文件未接受，合并后可能存在一致性问题",
  mergeSummaryCommit: "✅ 执行合并",
  mergeSummaryCancel: "❌ 取消",
  helpNoMembers: "暂无成员",
  helpMemberUser: "**用户**",
  helpMemberRole: "**角色**",
  helpSelectRole: "选择角色",
  ...
  mergeStatusConflict: "⚠️ 冲突",
  mergeStatusAdded: "🆕 新增",
  mergeStatusDeleted: "🗑️ 删除",
  mergeDiffTruncated: "\n... (truncated)",
  mergeDecisionKeepMainLabel: "保留基线",
  mergeDecisionUseBranchLabel: "使用分支",
  mergeDecisionSkipLabel: "跳过",
  mergeSummaryTitle: "合并汇总",
  mergeSummarySubtitle: (branchName, baseBranch, accepted, total) => `${branchName} → ${baseBranch} · ${accepted}/${total} 文件接受`,
...
const enUS: FeishuCardBuilderStrings = {
  ...
  mergeReviewRejectAction: "❌ Reject + send to agent",
  mergeReviewAcceptAll: (remaining) => `**Accept all** (${remaining} files remaining)`,
  mergeReviewProgress: (accepted, rejected, remaining, pct) => `Progress: ✅${accepted}  ❌${rejected}  ⏳${remaining} (${pct}%)`,
  mergeSummaryAccepted: (count, list) => `**Accepted (${count})**\n${list}`,
  mergeSummaryOthers: (count, list) => `**Others (${count})**\n${list}`,
  mergeSummaryPartialWarning: "⚠️ Some files were not accepted. The merge may leave consistency issues.",
  mergeSummaryCommit: "✅ Commit merge",
  mergeSummaryCancel: "❌ Cancel",
  ...
  mergeDecisionAccept: "Accept merge",
  ...
  mergeStatusConflict: "⚠️ Conflict",
  mergeStatusAdded: "🆕 Added",
  mergeStatusDeleted: "🗑️ Deleted",
  mergeDiffTruncated: "\n... (truncated)",
  ...
  mergeSummarySubtitle: (branchName, baseBranch, accepted, total) => `${branchName} → ${baseBranch} · ${accepted}/${total} files accepted`,
