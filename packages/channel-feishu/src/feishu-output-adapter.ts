...
export class FeishuOutputAdapter implements IMOutputAdapter {
  ...
  /** @deprecated 历史入口已统一走 TurnDetail 恢复；仅保留给旧测试/兼容代码。 */
  renderTurnCardFromStore(chatId: string, turnId: string): Record<string, unknown> | null {
    const state = this.turnCard.getCachedState(chatId, turnId);
    if (!state) return null;
