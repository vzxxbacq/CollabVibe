...
import type { UnifiedAgentEvent, UnifiedAgentTool } from "../../agent-core/src/unified-agent-event";
 ...
 */
export function createAcpEventFilter(): (update: AcpSessionUpdate) => UnifiedAgentEvent[] {
  ...
  return (update: AcpSessionUpdate): UnifiedAgentEvent[] => {
    ...
    const evt = acpEventToUnifiedAgentEvent(update);
    return evt ? [evt] : [];
  };
}

export function acpEventToUnifiedAgentEvent(update: AcpSessionUpdate): UnifiedAgentEvent | null {
  // opencode sends event type in 'sessionUpdate' field, fallback to 'type'
  const eventType = String(update.sessionUpdate ?? update.type ?? "");
  switch (eventType) {
