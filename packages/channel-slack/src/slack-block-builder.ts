    ...
    for (const action of req.availableActions) {
        if (action === "approve") {
            buttonDefs.push({
                ...
                value: JSON.stringify({ action: "approve", callId: req.callId, turnId: req.turnId }),
                style: "primary"
            });
        } else if (action === "deny") {
            buttonDefs.push({
                text: "❌ Deny",
                actionId: "codex_deny",
                value: JSON.stringify({ action: "deny", callId: req.callId, turnId: req.turnId }),
                style: "danger"
            });
            ...
            buttonDefs.push({
                ...
                value: JSON.stringify({ action: "approve_always", callId: req.callId, turnId: req.turnId })
            });
        }
    }
