...
interface BuildBatchMergeAgentPromptInput {
  worktreeCwd: string;
  files: string[];
  userPrompt?: string;
}

export async function buildBatchFileMergeAgentPrompt(
  input: BuildBatchMergeAgentPromptInput
): Promise<string> {
  const fileSections: string[] = [];
  for (const filePath of input.files) {
    const excerpt = await readFileExcerpt(input.worktreeCwd, filePath);
    fileSections.push(
      `### File: ${filePath}`,
      "```",
      excerpt,
  ...
  return [
    "You are a senior software engineer resolving git merge conflicts across multiple files.",
    "Your task is to produce the correct merged version for each target file while preserving intended behavior.",
    "",
    "Rules:",
    `- Only modify these files: ${input.files.map(f => `\`${f}\``).join(", ")}`,
    "- Do not edit any other file.",
    "- Remove all git conflict markers: <<<<<<<, =======, >>>>>>>",
    "- Preserve each file's purpose, public behavior, and useful comments/docstrings.",
    "- Merge both sides when they are compatible; do not drop required behavior just to make the conflict disappear.",
    "- When you finish editing each file, run git add on it.",
    "",
    ...(userPrompt ? [
      "Additional user instructions:",
      userPrompt,
