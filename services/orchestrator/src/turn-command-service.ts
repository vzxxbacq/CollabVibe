...
export class TurnCommandService extends TurnServiceBase {
  ...
  async recordTurnStart(input: RecordTurnStartInput): Promise<void> {
    const storageProjectId = this.storageProjectId(input.projectId, input.chatId);
    const snapshotSha = await (this.commandDeps.createSnapshot ?? createSnapshot)(input.cwd);
    const createdAt = this.deps.nowIso();
    const threadRecord = this.deps.threadService.getRecord(storageProjectId, input.threadName);
    ...
    await this.deps.threadService.markTurnRunning(storageProjectId, input.threadName, input.turnId);
    ...
    if (this.commandDeps.snapshotRepo) {
      try {
        ...
        const turnIndex = (await this.commandDeps.snapshotRepo.getLatestIndex(storageProjectId, input.threadId)) + 1;
