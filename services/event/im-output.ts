// ─────────────────────────────────────────────────────────────────────────────
// IM Output Data Types — Agent 事件 → 平台渲染 数据载荷
// ─────────────────────────────────────────────────────────────────────────────
//
// 设计目标：
...
//
// 数据流：
//   Backend Event → L3 EventBridge → UnifiedAgentEvent → transformer.ts → IMOutputMessage
//     → AgentEventRouter → toPlatformOutput() → PlatformOutput
//       → OutputGateway.dispatch(projectId, PlatformOutput)
//         → FeishuOutputGateway / SlackOutputGateway (内部渲染)
//
// 纯类型文件：无运行时代码，0% 覆盖率是预期的。
// ─────────────────────────────────────────────────────────────────────────────

import type { DiffFileSummary, DiffFileSegment } from "../../packages/git-utils/src/index";
import type { BackendCatalogView } from "../backend/backend-service";
...
import type { MergeFileStatus, MergeFileDecision } from "../../packages/git-utils/src/index";

/** Re-export from L3 git-utils — canonical definitions live there */
export type { MergeFileStatus, MergeFileDecision } from "../../packages/git-utils/src/index";
...
/** transformer.ts 的输出类型 — AgentEventRouter 使用此类型做 switch dispatch */
...
/** Path B merge event — unified merge router payload */
export interface IMMergeEventMessage {
  kind: "merge_event";
  data:
