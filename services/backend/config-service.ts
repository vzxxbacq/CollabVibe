...
import {
    parseClaudeConfigContent,
    parseCodexConfigContent,
...
export class BackendConfigService {
    ...
    async readCodexConfig(): Promise<BackendConfigInfo> {
        ...
        try {
            ...
            const { parsed, root, activeProvider, policy, providerMap } = parseCodexConfigContent({
                content,
                getExistingStatuses: (providerName) => this.getModelStatuses(source.name, providerName),
            });
            info.activeProvider = activeProvider;
            info.policy = policy;
    ...
    async readClaudeConfig(): Promise<BackendConfigInfo> {
        ...
        try {
            const content = readFileSync(localPath, "utf-8");
            const { activeProvider, providers } = parseClaudeConfigContent({
                content,
                getExistingStatuses: (providerName) => this.getModelStatuses(source.name, providerName),
            });
            info.activeProvider = activeProvider;
            info.providers.push(...providers);
            for (const provider of providers) {
                this.setModelStatuses(source.name, provider.name, provider.models);
            }
    ...
    async readOpencodeConfig(): Promise<BackendConfigInfo> {
        ...
        try {
            const content = readFileSync(localPath, "utf-8");
            const { parsed, policy, providers } = parseOpenCodeConfigContent({
                content,
                getExistingStatuses: (providerName) => this.getModelStatuses(source.name, providerName),
                resolveApiKey: (name) => this.resolveApiKey(name),
            });
            info.policy = policy;
            info.providers.push(...providers);
            for (const provider of providers) {
