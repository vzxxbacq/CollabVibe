import type { BackendIdentity, TransportType } from "../../packages/agent-core/src/index";
...
export interface BackendSessionResolver {
    /** Resolve backend for a project, optionally for a specific thread */
    resolve(projectId: string, threadName?: string): Promise<ResolvedBackendSession>;
...
export class DefaultBackendSessionResolver implements BackendSessionResolver {
    ...
    async resolve(projectId: string, threadName?: string): Promise<ResolvedBackendSession> {
        ...
        if (threadName) {
            ...
            if (!backendDef) {
                throw new Error(`backend definition missing for thread backend: backendId=${backend.backendId} projectId=${projectId} threadName=${threadName}`);
            }
