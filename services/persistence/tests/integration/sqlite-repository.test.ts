...
import { SqliteTurnDetailRepository } from "../../src/sqlite-turn-detail-repository";
import { SqliteTurnRepository } from "../../src/sqlite-turn-repository";
...
describe("sqlite repository integration", () => {
  ...
  it("does not delete turn_details when turn_records are updated", async () => {
    ...
    await turnDetailRepository.create({
      projectId: "proj-1",
      turnId: "turn-1",
      backendName: "codex",
      modelName: "gpt-5",
    ...
    await expect(turnDetailRepository.getByTurnId("proj-1", "turn-1")).resolves.toEqual(
