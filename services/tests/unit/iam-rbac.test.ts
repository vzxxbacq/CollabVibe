...
describe("IAM and RBAC", () => {
  ...
  it("addAdmin makes user an admin", async () => {
    ...
    sim.api.addAdmin("new-admin");
    expect(sim.api.isAdmin("new-admin")).toBe(true);
  });

  it("removeAdmin revokes admin", async () => {
    sim = await SimHarness.create(["root"]);
    sim.api.addAdmin("temp-admin");
    sim.api.removeAdmin("temp-admin");
  ...
  it("removeAdmin on env admin preserves it (env admins are undeletable)", async () => {
    sim = await SimHarness.create(["env-admin"]);
    const result = sim.api.removeAdmin("env-admin");
  ...
  it("admin can add project member", async () => {
    ...
    sim.api.addProjectMember({ projectId, userId: "dev-1", role: "developer", actorId: "owner" });
  ...
  it("add multiple members with different roles", async () => {
    ...
    sim.api.addProjectMember({ projectId, userId: "maint", role: "maintainer", actorId: "owner" });
    sim.api.addProjectMember({ projectId, userId: "aud", role: "auditor", actorId: "owner" });
    const members = sim.api.listProjectMembers(projectId);
  ...
  it("removeProjectMember removes from list", async () => {
    ...
    sim.api.addProjectMember({ projectId, userId: "temp", role: "developer", actorId: "owner" });
    sim.api.removeProjectMember({ projectId, userId: "temp", actorId: "owner" });
    const members = sim.api.listProjectMembers(projectId);
  ...
  it("removeProjectMember for non-member is safe", async () => {
    ...
    // Should not throw
    sim.api.removeProjectMember({ projectId, userId: "ghost-user", actorId: "owner" });
  });
  ...
  it("non-admin cannot remove project member", async () => {
    ...
    sim.api.addProjectMember({ projectId, userId: "victim", role: "developer", actorId: "owner" });
    expect(() => {
  ...
  it("updateProjectMemberRole changes role", async () => {
    ...
    sim.api.addProjectMember({ projectId, userId: "changeme", role: "developer", actorId: "owner" });
  ...
  it("updateProjectMemberRole with non-admin denied", async () => {
    ...
    sim.api.addProjectMember({ projectId, userId: "target", role: "developer", actorId: "owner" });
    expect(() => {
  ...
  it("updateProjectMemberRole to same role is idempotent", async () => {
    ...
    sim.api.addProjectMember({ projectId, userId: "same", role: "developer", actorId: "owner" });
  ...
  it("resolveRole for admin returns admin", async () => {
    ...
    const role = sim.api.resolveRole({ userId: "admin" });
  ...
  it("resolveRole for developer returns developer", async () => {
    ...
    sim.api.addProjectMember({ projectId, userId: "dev-rr", role: "developer", actorId: "owner" });
    const role = sim.api.resolveRole({ userId: "dev-rr", projectId });
    expect(role).toBe("developer");
  });

  it("resolveRole for unknown user returns non-admin role or null", async () => {
    sim = await SimHarness.create(["owner"]);
    const role = sim.api.resolveRole({ userId: "unknown-user" });
  ...
  it("resolveRole for different projects", async () => {
    ...
    sim.api.addProjectMember({ projectId: pid1, userId: "multi-user", role: "developer", actorId: "owner" });
    sim.api.addProjectMember({ projectId: pid2, userId: "multi-user", role: "maintainer", actorId: "owner" });
    expect(sim.api.resolveRole({ userId: "multi-user", projectId: pid1 })).toBe("developer");
    expect(sim.api.resolveRole({ userId: "multi-user", projectId: pid2 })).toBe("maintainer");
  });
  ...
  it("addProjectMember auto-registers user", async () => {
    ...
    sim.api.addProjectMember({ projectId, userId: "new-dev", role: "developer", actorId: "owner" });
  ...
  it("admin manages multiple projects", async () => {
    ...
    sim.api.addProjectMember({ projectId: pid1, userId: "user-x", role: "developer", actorId: "admin" });
    sim.api.addProjectMember({ projectId: pid2, userId: "user-x", role: "auditor", actorId: "admin" });
    expect(sim.api.listProjectMembers(pid1).some((m) => m.userId === "user-x" && m.role === "developer")).toBe(true);
    expect(sim.api.listProjectMembers(pid2).some((m) => m.userId === "user-x" && m.role === "auditor")).toBe(true);
  });
});
