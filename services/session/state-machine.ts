...
export type ConversationState = "IDLE" | "RUNNING" | "AWAITING_APPROVAL" | "INTERRUPTING" | "INTERRUPTED" | "FAILED";

const ALLOWED_TRANSITIONS: Record<ConversationState, ConversationState[]> = {
  IDLE: ["RUNNING", "FAILED"],
  RUNNING: ["IDLE", "AWAITING_APPROVAL", "INTERRUPTING", "FAILED"],
  AWAITING_APPROVAL: ["RUNNING", "INTERRUPTING", "FAILED"],
...
export class ApprovalWaitManager {
  ...
  clear(approvalId: string): boolean {
    const pending = this.pending.get(approvalId);
    if (!pending) {
      return false;
    }
